...
public class MapViewModel : ViewModel
{
	...
	public MapViewModel(BaseServices services, MySqliteConnection data, IGpsManager gpsManager,
		IBackendClient backend, AppSettings settings, INotificationManager notifications)
		: base(services)
	{
		this.data = data;
	...
	private async Task RefreshAllUserLocations()
	{
		...
		try
		{
			...
			try
			{
				var activeJobs = (await backend.GetJobsAsync())
			...
			foreach (var profile in profilesDict.Values)
			{
				...
				// Trail line — only when actively online/moving, not when offline or stale
				if (userPings != null && userPings.Count >= 2 && (status == "online" || status == "busy"))
				{
					...
					var sessionPings = new List<BackendPing>();
	...
	public async Task AssignJobToUser(string userId)
	{
		...
		if (result.HasValue)
		{
			try
			{
				await backend.CreateJobAsync(userId, result.Value.Title, result.Value.Description);
				await Dialogs.Alert("Job assigned successfully.", "Done");
			}
			catch
			{
				await Dialogs.Alert("Failed to assign job. Please try again.", "Error");
			}
		}
	}
	...
	private async Task<bool> CheckIsAdmin()
	{
		try
		{
			...
			return (await backend.GetProfileAsync(userId))?.IsAdmin ?? false;
		}
		catch { return false; }
	}
